...
import dfinfo from './dfinfo.json'
...
class DevilFruit extends Component {
  ...
  constructor (props) {
    ...
    this.state = {
      searchQuery: '',
      searchResults: dfinfo.items.slice(0, 3)
    }
  }
  ...
  submitSearch = () => {
    browserHistory.push('/DevilFruit')
  }

  performSearch (query) {
    return dfinfo.items.filter((item) => {
  ...
  render () {
    const dfinfo = this.props.searchResults.map((dfinfo, index) => {
      return (
        <div className='DV' key={index}>
          <h1>{dfinfo.title}</h1>
          <div className='characterBio'>
          <img className='bioImage' src={dfinfo.image} />
            <div className='bio'>{dfinfo.snippet.split('\n\n').map((text) => {
              return <p>{text}</p>
            })}</div>
          </div>
        </div>
